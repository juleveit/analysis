function sizeanalysis_SU_allcells_simple
    ...
    for ori = 1:length(oris)
        for sz = 1:length(sizes)
            ...
            for i = 1:length(thisinds)                
                spks = find(cllresp(cll,i,respwin));
                lfpmat = zeros(length(spks),501);
                if ~isempty(spks)
                    for ss = 1:length(spks)
                        lfpmat(ss,:) = clllfpresp(loc,thisinds(i),(spks(ss)+respwin(1)-250):(spks(ss)+respwin(1)+250));
                    end
            ...
            condstalfp(l,ori,sz,:) = nanmean(stalfp,1);
        end
    end
end
...
% cross correlations
cll1 = 2; cll2 = 17;
a = find(gratingInfo.size == 8);
b = find(gratingInfo.size == 60);
